...
def _to_year_list(cols: Iterable):
    ...
    for c in cols:
        y = None
        try:
            # PeriodIndex or Timestamp
            if hasattr(c, "year"): y = int(c.year)
            # string like '2024-09-30' or '2024'
            elif isinstance(c, str):
                # grab first 4 consecutive digits
                import re
                m = re.search(r"(\d{4})", c)
                if m: y = int(m.group(1))
            # plain int
            elif isinstance(c, (int, np.integer)):
                if 1900 <= int(c) <= 2100: y = int(c)
        except Exception:
            pass
        years.append(y)
    return years
...
def fetch_statements(ticker):
    ...
    def tidy(df):
        ...
        # Derive years robustly regardless of column type
        years = _to_year_list(df.columns)
        df.columns = years
        # Keep valid years only
        keep = [i for i, y in enumerate(years) if isinstance(y, int)]
        if not keep:
            return pd.DataFrame()
        df = df.iloc[:, keep]
        ...
        # Drop duplicate year columns keeping latest
...
def pick(df, names):
    """Find a row by any of the provided names (case/space insensitive, with substring fallback)."""
    if df is None or df.empty:
...
def compute_series(ticker):
    ...
    # Broad labels (cover tech/industrial + banks/insurers)
