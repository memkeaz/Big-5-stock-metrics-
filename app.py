...
st.caption("Auto source (Alpha Vantage → FMP → Yahoo), Big 5 with 10/5/3/1 breakdowns, and two valuations: Rule #1 EPS & FCF‑DCF. One MOS slider applies to both.")
...
sset("years_eps", 10)
sset("growth_eps_user", 0.12)
sset("auto_pe", True)
sset("terminal_pe_man", 20.0)
sset("years_dcf", 10)
sset("growth_fcf", 0.10)
sset("terminal_g", 0.03)
sset("mos_pct", 50)
sset("last_query", {"ticker": "ADBE", "provider": "Auto (AV→FMP→Yahoo)"})
sset("data", None)               # (df, years, source, diag, current_price, provider_label)
...
def fetch_yahoo(symbol):
    ...
    # EPS fallback (NI / shares) if needed
    if (eps.dropna().le(0).all()) and (shares.notna().any()) and (net_income.notna().any()):
    ...
    df = pd.DataFrame({
        ...
        "FCF": fcf, "ROIC": roic, "SharesDiluted": shares
    }).sort_index().tail(11)
...
def _fmp_build_df(inc, bal, cfs):
    ...
    if (eps.dropna().le(0).all()) and (net_income.notna().any()) and (diluted_shares.notna().any()):
        with np.errstate(invalid="ignore", divide="ignore"):
    ...
    df = pd.DataFrame({
        "Revenue": revenue, "NetIncome": net_income, "EPS": eps, "Equity": equity,
        "FCF": fcf, "ROIC": roic, "SharesDiluted": diluted_shares
    }).sort_index().tail(11)
...
def fetch_fmp(symbol, apikey):
    ...
    try:
        ...
        if len(years) >= 10: return df, years, "FMP (annual)", diag
    except Exception:
        pass
    # 2) timescale=yearly (some accounts)
    try:
        ...
        df, years, diag = _fmp_build_df(inc, bal, cfs)
        if len(years) >= 10: return df, years, "FMP (yearly)", diag
    except Exception:
        pass
    # 3) as-reported (often deeper)
...
def av_get(fn, symbol, apikey):
    ...
    if isinstance(j, dict):
        if j.get("Note"):         raise RuntimeError("Alpha Vantage rate limit hit. Wait or switch provider.")
...
def fetch_alpha_vantage(symbol, apikey):
    ...
    if not inc_a and not bal_a and not cfs_a:
        raise RuntimeError("Alpha Vantage returned no annual data.")
    ...
    if not years:
        raise RuntimeError("Alpha Vantage fundamentals contain no annual years.")
    ...
    eps = diluted_eps.copy()
    if (eps.dropna().le(0).all()) and (net_income.notna().any()) and (shares_diluted.notna().any()):
        with np.errstate(invalid="ignore", divide="ignore"):
    ...
    diag = {"annual_reports_counts": {"income": len(inc_a), "balance": len(bal_a), "cashflow": len(cfs_a)},
...
# =============== Auto chooser (Alpha → FMP → Yahoo) ===============
def try_provider(fn_fetch, fn_price):
    try:
        df, years, source, diag = fn_fetch()
        # must have at least some EPS or FCF data to be usable
        ok_eps = ("EPS" in df) and (df["EPS"].notna().sum() >= 2)
        ok_fcf = ("FCF" in df) and (df["FCF"].notna().sum() >= 2)
        if df is not None and not df.empty and len(df.index) >= 3 and (ok_eps or ok_fcf):
...
# =============== Search form (fetch only on click) ===============
with st.form("search"):
    ...
    with c2:
        provider = st.selectbox(
            "Data source",
            ["Auto (AV→FMP→Yahoo)", "Alpha Vantage", "FMP", "Yahoo Finance"],
            index=["Auto (AV→FMP→Yahoo)", "Alpha Vantage", "FMP", "Yahoo Finance"].index(st.session_state["last_query"]["provider"])
        )
...
if go:
    try:
        if provider.startswith("Auto"):
            ...
            if not AV_KEY:
                st.error("Missing ALPHAVANTAGE_API_KEY in Secrets."); st.stop()
            df, years, source, diag = fetch_alpha_vantage(ticker, AV_KEY)
            current_price = get_price_av_safe(ticker)
        elif provider == "FMP":
            if not FMP_KEY:
                st.error("Missing FMP_API_KEY in Secrets."); st.stop()
...
# =============== Valuation settings (LIVE widgets, no refetch) ===============
with st.expander("Valuation Settings", expanded=False):
    ...
    st.session_state["growth_eps_user"] = b1.number_input("Your EPS growth %", 0.0, 50.0, float(st.session_state["growth_eps_user"]*100), 0.5) / 100.0
    st.session_state["auto_pe"]         = b2.checkbox("Terminal P/E = min(2×growth%, current P/E, 50)", value=bool(st.session_state["auto_pe"]))
    st.session_state["terminal_pe_man"] = b3.number_input("Terminal P/E (manual if Auto off)", 5.0, 60.0, float(st.session_state["terminal_pe_man"]), 0.5)

    c1, c2, c3 = st.columns(3)
    st.session_state["growth_fcf"] = c1.number_input("FCF growth %", 0.0, 50.0, float(st.session_state["growth_fcf"]*100), 0.5) / 100.0
    st.session_state["terminal_g"] = c2.number_input("Terminal growth % (FCF)", 0.0, 6.0, float(st.session_state["terminal_g"]*100), 0.25) / 100.0
...
if st.session_state["data"] is None:
    st.info("Enter a ticker and click **Search** to load data. (Auto tries Alpha → FMP → Yahoo.)")
    st.stop()
...
with tabs[3]:
    ...
    # --- Rule #1 EPS (use best EPS: named EPS OR NI/Shares) ---
    eps_series = df["EPS"].astype(float)

    eps_alt = pd.Series(dtype="float64")
    if ("NetIncome" in df.columns) and df["NetIncome"].notna().any() and df["SharesDiluted"].notna().any():
        with np.errstate(invalid="ignore", divide="ignore"):
            eps_alt = (df["NetIncome"] / df["SharesDiluted"].replace({0: np.nan})).astype(float)

    eps_best = eps_series.copy()
    if not eps_alt.empty:
        eps_best = eps_best.fillna(eps_alt)

    def _latest_pos(s, lookback=10):
        s = s.dropna()
        if lookback: s = s.iloc[-lookback:]
        s = s[s > 0]
        return s.iloc[-1] if len(s) else np.nan

    last_eps = _latest_pos(eps_best, lookback=10)
    eps_hist_cagr = series_cagr_gap(eps_best)
    ...
    if pd.isna(fair_rule1):
        st.warning("Rule #1 fair value unavailable — need positive EPS (best of EPS or NI/Shares). Try a different provider or Auto.")
    if pd.isna(iv_dcf):
        st.warning("DCF fair value unavailable — need positive FCF/share (CFO − |CapEx|, per share). Try a different provider or Auto.")
...
with tabs[4]:
    ...
    st.caption("If EPS/FCF are missing on one provider, try Auto mode or switch providers.")
